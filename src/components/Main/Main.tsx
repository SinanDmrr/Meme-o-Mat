...
//         useCORS: true, // Für Bilder von externen URLs
//         backgroundColor: null, // Transparenter Hintergrund
//       });
//       const link = document.createElement("a");
//       link.href = canvas.toDataURL("image/png"); // Als PNG speichern
...
//           <img
//             src={trollFace}
//             className="w-8 h-8 scale-x-[-1] xs:w-16"
...
//         </button>
//       </div>
//       <div className="meme w-full max-w-[570px] min-w-[300px] my-5 relative flex-1">
//         <img
...
export default function Main({memeUrlArray}: MainProps) {
  ...
  const [imageWidth, setImageWidth] = useState<number | null>(null);
  const [imageHeight, setImageHeight] = useState<number | null>(null); // Neue State-Variable für die Bildhöhe
  const memeRef = useRef<HTMLDivElement>(null);
  const imgRef = useRef<HTMLImageElement>(null); // Ref für das Bild
  ...
  function handleGenerateMeme() {
    ...
    setImageWidth(null);
    setImageHeight(null); // Höhe zurücksetzen
  }
  ...
  function handleImageLoad(event: React.SyntheticEvent<HTMLImageElement>) {
    const img = event.currentTarget;
    setImageWidth(img.clientWidth);
    setImageHeight(img.clientHeight); // Speichere die tatsächliche Bildhöhe
  }
  ...
  return (
    <main className="flex-1 flex flex-col items-center justify-start w-full px-5">
      ...
      <div
        ...
        ref={memeRef}
        style={{height: imageHeight ? `${imageHeight}px` : "auto"}} // Höhe des Containers an Bildhöhe anpassen
      >
        <img
          ref={imgRef}
          src={memeInformation.imgUrl}
          ...
          onLoad={handleImageLoad}
          crossOrigin="anonymous" // Versuch, CORS zu umgehen
        />
